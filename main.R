...
brm = function(y, x, va, vb = NULL, param, est.method = "MLE", vc = NULL, 
    ...
    alpha.start = NULL, beta.start = NULL, message = FALSE, method = "brm", CI = "wald") {
    ...
    if (est.method == "MLE"){
      if(method == "brm"){
        sol = MLEst(param, y, x, va, vb, weight, max.step, thres, alpha.start,
                    beta.start, pa, pb, method = "brm", CI)
        Na0 <- sum(x==0)
        Na1 <- sum(x==1)
        ...
        if(P0==0|P0==1|P1==0|P1==1) {
          cat("Zero incidence appears")
          bayes_est = if (param == "RR") bayes_est_RR else bayes_est_RD
          est.bayes = bayes_est(Na0,Na1,N0_1,N1_1)
          sol$point.est[1] = est.bayes$point.est
          sol$se.est[1] = est.bayes$se.est
          sol$conf.lower[1] = est.bayes$conf.lower
          sol$conf.upper[1] = est.bayes$conf.upper
          sol$p.value[1] = est.bayes$p.value
        }
      }else{
        sol = MLEst(param, y, x, va, vb, weight, max.step, thres, alpha.start,
                    beta.start, pa, pb, method, CI)
      }
    }
    if (est.method == "DR") {
        if (param == "OR") {
